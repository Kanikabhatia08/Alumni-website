    ...
    <div className="container bg-gray-200 mx-auto mt-20 w-full h-full">
      ...
      <div className="relative wrap overflow-hidden p-10 h-full">
        ...
        {eventData.map((item, index) => (
          <div
            key={index}
            className={` flex justify-between items-center w-full ${item.position === 'right' ? 'right-timeline' : 'left-timeline flex-row-reverse'}`}
          >
            ...
            <div className={`order-1 ${item.bgColor} flex flex-col gap-3 rounded-lg shadow-xl w-5/12 px-6 py-4`}>
              <div>
                ...
                <p className="text-lg leading-snug tracking-wide" style={{ color: item.textColor }}>{item.description}</p>
              </div>
              <div className=''>
